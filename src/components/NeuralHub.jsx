...
const NeuralHub = ({ 
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
            ...
            return (
              <motion.button
                ...
                className={`
                  ...
                  ${isSelected
                    ? 'bg-yellow-500 text-black border-yellow-500 scale-105 z-10 italic shadow-[0_0_20px_rgba(234,179,8,0.4)]'
                    : isUnlocked
                ...
                <AnimatePresence>
                  {isUnlocked && (
                    <motion.div 
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="text-[7px] font-black leading-none mt-1.5 bg-[#00ff88]/20 px-1.5 py-0.5 rounded-sm"
                    >
                      SYNC
                    </motion.div>
                  )}
      ...
      >
        <div className="glass-card bg-gradient-to-br from-yellow-500/10 via-yellow-500/5 to-transparent border border-yellow-500/20 p-8 rounded-[3rem] relative overflow-hidden group h-full">
          <div className="absolute top-0 right-0 p-8 opacity-[0.03] text-yellow-500 pointer-events-none group-hover:opacity-[0.06] transition-opacity duration-1000">
            <ShieldCheck className="w-48 h-48" />
          </div>

          <div className="flex items-center gap-5 mb-10 relative z-10 border-b border-white/5 pb-6">
            <div className="w-12 h-12 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl flex items-center justify-center shadow-lg shadow-yellow-500/5">
              <Zap className="w-6 h-6 text-yellow-500 animate-pulse" />
            </div>
            <div>
              <h2 className="text-2xl font-black text-white tracking-tight uppercase italic leading-none">Authentication</h2>
              <p className="text-[10px] font-black text-yellow-500/60 uppercase tracking-[0.3em] italic mt-2">Neural Authority Matrix</p>
            </div>
          </div>
          ...
          <div className="space-y-8 relative z-10">
            <div className="bg-black/60 backdrop-blur-md border border-white/10 rounded-[2.5rem] p-10 text-center space-y-4 shadow-2xl relative overflow-hidden">
               ...
               <div className="text-[11px] font-black text-white/40 uppercase tracking-[0.4em] mb-2 italic">Expansion Cost</div>
               <div className="text-5xl font-black text-yellow-500 tracking-tighter italic drop-shadow-sm">
                  {bnbAmount.toFixed(4)} <span className="text-lg text-white/60 uppercase italic ml-1 tracking-widest font-black transition-colors group-hover:text-[#00ccff]">BNB</span>
               </div>
               <div className="inline-flex items-center px-5 py-2 bg-white/5 rounded-full border border-white/5 gap-2 group-hover:border-yellow-500/20 transition-all">
                  <span className="w-1.5 h-1.5 bg-yellow-500 rounded-full animate-pulse" />
                  <span className="text-[11px] font-black text-white/60 uppercase tracking-[0.2em] italic">Protocol Value: ~${usdAmount}</span>
               </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/5 border border-white/5 rounded-3xl p-5 hover:bg-white/[0.07] transition-all">
                <span className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] block mb-2">Target Expansion</span>
              ...
              <div className="bg-white/5 border border-white/5 rounded-3xl p-5 hover:bg-white/[0.07] transition-all">
                <span className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] block mb-2">Strategic Layers</span>
                <span className="text-lg font-black text-yellow-500 italic">+{layersToUpgrade} Units</span>
              </div>
            </div>
            ...
            <motion.button
              ...
              className={`
                ...
                ${isNext ? 'bg-yellow-500 text-black shadow-yellow-500/20' : 'bg-white/10 text-white/20'}
              `}
            >
